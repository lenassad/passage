...
def add_adaptive_laplace_noise(mat, degrees, epsilon):
    n = mat.shape[0]
    noisy_mat = mat.copy().astype(float)

    # 获取非零上三角索引
    rows, cols = np.triu_indices(n, k=1)
    nonzero_mask = (mat[rows, cols] > 0)
    rows = rows[nonzero_mask]
    cols = cols[nonzero_mask]

    # 矢量化计算 scale 和 noise
    deg_sum = degrees[rows] + degrees[cols]
    scales = 1 / (epsilon * np.log(1 + deg_sum + 1e-1))
    noises = np.random.laplace(loc=0, scale=scales)

    noisy_mat[rows, cols] += noises
    noisy_mat[cols, rows] += noises

    return noisy_mat


# 新：低度节点吸附到社区中心节点
def absorb_low_degree_nodes(mat, communities, min_degree=2):
    mat = mat.copy()
    n = len(mat)
    ...
    for idx, com in enumerate(communities):
        community_dict.setdefault(com, []).append(idx)

    for i in range(n):
        if degrees[i] < min_degree:
            com = communities[i]
            members = community_dict[com]
            if len(members) <= 1:
                continue  # 跳过社区规模为1的情况

            # 找出度数最大的社区成员（非自身）
            max_deg = -1
            center_node = -1
            for j in members:
                if j != i and degrees[j] > max_deg:
                    max_deg = degrees[j]
                    center_node = j

            # 若未连边则添加
            if center_node >= 0 and mat[i, center_node] == 0:
                mat[i, center_node] = 1
                mat[center_node, i] = 1
                degrees[i] += 1
                degrees[center_node] += 1
...
def community_init(mat0, mat0_graph, epsilon, nr, t=1.0):
    ...
    # 将低度节点吸附到其社区中心
    mat_after_absorb = absorb_low_degree_nodes(mat0, communities, min_degree=2)

    # 可再次添加噪声
    final_noisy_graph = add_adaptive_laplace_noise(mat_after_absorb, degrees, epsilon)
